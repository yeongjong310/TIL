...
## 1. compiler Theory
자바스크립트가 인터프리터 언어로 알려져있지만, 자바스크립트는 컴파일 언어이기도하다. 하지만 일반적인 컴파일 언어와는 다른점이있다.
...
이러한 트리는 상위 엘리먼트에서 부터 하위 엘리먼트로 이어지는 노드로 구성되고 "AST(Abstract Syntax Tree)" 라고 불린다.
3. Code-Generation:
...
### 2.2. Compier Speak
3단계에서 코드가 실행되고 엔진은 변수를 찾는다. 하지만 이때 엔진이 사용하는 검색 종류에 따라 결과가 달라진다.
#### 검색 종류
1. LHS(검색 + 할당) : 변수가 대입 연산자의 왼쪽에 있을 때 수행 ex) `a = 10`
2. RHS(검색 + 반환) : 변수가 대입 연산자의 오른쪽에 있을 때 수행   `console.log( a )`
